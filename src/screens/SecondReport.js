...
//     }
// }
export class SecondReport extends Component {
  state = {
    location: "",
    ASN: [],
    device: "",
  ...
  dateFilter = () => {
    return this.state.allData.filter(
      (x) =>
        x?.assetDetails[0]["Thing Name"]
          ?.toLowerCase()
          .includes(this.state?.ibt.toLowerCase())
        &&

        x?.asset_EPC
          ?.toLowerCase()
          .includes(this.state?.startingDate.toLowerCase())
        &&
        x?.assetDetails[0]?.Serial_no?.includes(
          this.state?.remarks
        )
        &&
  ...
  runFunction = async () => {
    ...
    this.setState({
      countedItems: countedItems.filter((i) => !!i.assetDetails.length),
      allData: countedItems.filter((i) => !!i.assetDetails.length),
    });
          ...
          <div className="main-dashboard">
            ...
            <div className="dashboard ">
              ...
              >
                ...
                <h1 className="dashboard-heading"> Counted (Report)</h1>
                <Button
                ...
                <IconButton style={{ position: "absolute", right: "90px", cursor: 'pointer' }}>
                  <CSVLink filename="Counted_Report" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: 60 }} data={data} headers={headers}>

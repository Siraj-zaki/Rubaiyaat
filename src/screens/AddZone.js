...
export class AddZone extends Component {
    ...
    async componentDidMount() {
        ...
        console.log(Role);
        // console.log();
        const Site = await api.getAllDepartment()
        this.setState({ Site })
        console.log(Site);
    }
    filterNameEvent = (e) => {
    ...
    onSubmitEvent(e) {
        e.preventDefault()
        let User = {
            zone_name: this.state.filterName,
            level: this.state.filterUsername,
            departement: this.state.Site.filter((item => item.departement_name === this.state.site.label)).map((site => {
                return site._id
            })).reduce((data => {
                return data
            })),
                        ...
                        <div className="dashboard ">
                            ...
                            <div style={{ width: '100%', display: 'flex', justifyContent: 'space-evenly', alignItems: 'center', backgroundColor: 'transparent' }}>
                                <form style={{ width: '80%', margin: 20, display: 'flex', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', rowGap: 20, columnGap: 70 }} onSubmit={(e) => this.onSubmitEvent(e)} noValidate={false} autoComplete="off">
                                    <BasicTextFields type={"text"} placeholder={"Zone Name"} require name={"Zone Name"} value={this.state.filterName} onChangeEvent={(e) => this.filterNameEvent(e)} />
                                    ...
                                    <div style={{ width: '100%', margin: 0, display: 'flex', justifyContent: 'center', alignItems: 'center', flexWrap: 'wrap', rowGap: 20, columnGap: 70 }}>
                                        <Select
                                            ...
                                            options={this.state.Site.filter((item => item.departement_name)).map((role => {
                                                return { label: role.departement_name }
                                            }))}
                                            isSearchable={true}
                                    ...
                                    <Button type="submit" variant="contained">Add Zone</Button>
                                </form>
                            </div>
                        </div>
                    </div>

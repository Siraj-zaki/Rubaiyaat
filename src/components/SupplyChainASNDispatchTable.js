    ...
    return (
        <React.Fragment>
            <TableRow className={classes.root}>
                ...
                <TableCell colSpan={2} style={{ color: 'white', fontWeight: 'bold', letterSpacing: 1, fontSize: 12, }} align="center"><Link to={{ pathname: "/EPCDetail", state: { row: { operation: 'transfer out', asn: row._id }, data: { zone: row?.zoneId?.zone_name, departmentName: row?.departementId?.departement_name } } }}>{row.transfer_items?.qt}</Link> </TableCell>
